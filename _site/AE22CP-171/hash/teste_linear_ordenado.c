...
//	int h = hashNome(nome, cap);
	...
	{
		if (thash[hvalue].matricula > matricula)
		{
			tmat = matricula;
			strcpy(tnome, nome);
				 
			matricula = thash[hvalue].matricula;
			strcpy(nome, thash[hvalue].nome);
			
			thash[hvalue].matricula = tmat;
			strcpy(thash[hvalue].nome, tnome);
		}
